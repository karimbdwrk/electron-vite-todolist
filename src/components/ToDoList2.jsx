...
const ToDoList2 = () => {
	...
	const fetchData = async () => {
		try {
			const response = await fetch(
				`http://localhost:1337/api/todolists/${id}?populate=*`,
				{
					method: "GET",
					headers: {
	...
	const handleEditIndex = (id) => {
		setEditIndex(id);
		const filtre = todolist.attributes.todos.data.filter(
			(todo) => todo.id === id
		);
	...
	const handleEditList = (e) => {
		...
		fetch(`http://localhost:1337/api/todolists/${id}`, {
			method: "PUT",
			headers: {
				"Content-type": "application/json",
				Authorization: `Bearer ${jwt}`,
